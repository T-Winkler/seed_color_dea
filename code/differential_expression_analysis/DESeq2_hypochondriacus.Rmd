...
# delete this later, check for what happens when the 587 sample is excluded due to the low number of reads
#design.matrix <- design.matrix[-c(17:18),]
#design.matrix <- droplevels(design.matrix)
#counts.in <- counts.in[,-c(17:18)]

# delete this later, plot PCA with complete sample names
...
# followed the instructions from bioconductor vignette
# first do pre filtering to only keep rows with at least 30 reads total
...
write_csv(myb_expression, file = "data/differential_expression_analysis/myb_expression.csv")
...
Create an volcano plot of the log fold change along with the p-values.
...
#annotation.raw <- read.table("/home/tom/Documents/master_thesis/data/functional_annotation/eggnog_mapper/MM_8wexw920.emapper.annotations.tsv",
#                             sep = "\t", comment.char = "#", fill=T)
# works correctly if eggNOG columns are written to a new file before loading them in
